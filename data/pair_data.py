...
class PairData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        hr = hr.resize(self.hr_size)
        img = img.resize(self.hr_size)
        if self.from_hr:
            lr = hr.resize((hr.size[0] // self.scale, hr.size[1] // self.scale))
        else:
        ...
        if self.suffix == 'tif':
            _min = np.iinfo('int16').min
            _max = np.iinfo('int16').max
            lr = torch.from_numpy((np.array(lr) - _min) / 65536).float().unsqueeze(0)
            hr = torch.from_numpy((np.array(hr) - _min) / 65536).float().unsqueeze(0)
